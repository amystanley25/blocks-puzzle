...
class Model {
    ...
    /**
     * Returns the width (number of columns of cells)
     * of the board.
     */
    int width() {
        return _width;
     ...
     * If all filled squares of the piece would overlap empty
     * squares on the board, this piece is placeable.
     */
    boolean placeable(Piece piece, int row, int col) {
        if (piece == null) {
            return false;
        }
        for (int pRow = 0; pRow < piece.height(); pRow++) {
            for (int pCol = 0; pCol < piece.width(); pCol++) {
                if (piece.get(pRow, pCol) && (get(row + pRow, col + pCol))) {
                    return false;
                }
            }
        }
    ...
    boolean roundOver() {
        for (int k = 0; k < handSize(); k++) {
            if (_hand.get(k) != null && placeable(_hand.get(k))) {
                return false;
            }
        }
        return true;
